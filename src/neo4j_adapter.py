...
class Neo4jAdapter:
    ...
    def define_schema(self):
        ...
        with self.driver.session() as session:
            ...
            # Load ship schema and inject spatial hierarchy
            try:
                import json
                with open("ship_schema.json", "r") as f:
                    schema = json.load(f)
                    for deck in schema["decks"]:
                        session.run("MERGE (d:Deck {id: $id}) SET d.name = $name", id=deck["id"], name=deck["name"])
                        for room in deck["rooms"]:
                            session.run("MERGE (r:Location {name: $name}) "
                                        "MERGE (d:Deck {id: $did}) "
                                        "MERGE (r)-[:PART_OF]->(d)", name=room, did=deck["id"])
            except FileNotFoundError:
                print("ship_schema.json not found, skipping spatial hierarchy injection.")
    ...
    def ingest_processed_data(self, processed_chapters: dict):
        ...
        # Load roles for job-role mapping
        roles_map = {}
        try:
            import json
            with open("ship_schema.json", "r") as f:
                schema = json.load(f)
                for group, titles in schema["roles"].items():
                    for title in titles: roles_map[title.lower()] = group
                ...
                for chunk in chunks:
                    for triple in chunk.get("triples", []):
                        ...
                        if pred_label in ["LOCATED_AT", "NOT_LOCATED_AT"]:
                            ...
                            # If the identity matches a known role, also tag it with HAS_ROLE
                            obj_name = triple.get("object", {}).get("text", "").lower()
                            if obj_name in roles_map:
                                role_group = roles_map[obj_name]
                                session.run("MERGE (e:Entity {canonical_id: $sid}) "
                                            "MERGE (r:Role {name: $role}) "
                                            "MERGE (e)-[:HAS_ROLE]->(r) "
                                            "SET r.group = $group", sid=subj_id, role=obj_name, group=role_group)
