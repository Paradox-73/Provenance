...
def extract_entities(text: str) -> list[dict]:
    ...
    for ent in doc.ents:
        ...
        entities.append({
            ...
            "provenance": {"source_text_snippet": text[ent.start_char:ent.end_char+20], "confidence": 0.95}
        })
    return entities

def get_subject_recursive(token):
    """
    Recursively searches for a subject if the current verb (xcomp) doesn't have one.
    """
    ...
    # 2. Check parent (head) if this is an open clausal complement (xcomp)
...
def extract_triples(text: str) -> list[dict]:
    """
    Advanced Triple Extraction with Recursive Subject Finding and Dual-Extraction.
    """
    if nlp is None: load_spacy_model()
    doc = nlp(text)
    triples = []
    ...
    for sent in doc.sents:
        for token in sent:
            if token.pos_ in ["VERB", "AUX"]:
                ...
                # A. Handle "Projective Verbs" (Locations)
                # This extracts: (Stone, located at, Console)
                ...
                # B. Standard Extraction (Inventory / Actions)
                # CHANGED: 'elif' -> 'if'. Now this runs EVEN IF block A ran.
                # This extracts: (He, placed, Stone)
                if subject_node:
                    # SVO
        ...
        for chunk in chunks:
            ...
            for triple in chunk['triples']:
                for role in ["subject", "object"]:
                    ...
                    if "canonical_id" in role_data and "UNRESOLVED" not in role_data["canonical_id"]: continue

                    best_match_id = None
                    min_distance = float('inf')
                    
                    for entity in chunk['entities']:
                        if "canonical_id" not in entity or "UNRESOLVED" in entity["canonical_id"]: continue
